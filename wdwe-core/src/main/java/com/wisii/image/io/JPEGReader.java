 ...
 *//* $Id: JPEGReader.java,v 1.1 2007/04/12 06:41:19 cvsuser Exp $ */
 ...
 * @version $Id: JPEGReader.java,v 1.1 2007/04/12 06:41:19 cvsuser Exp $
 */
public class JPEGReader implements ImageReader {
    ...
    public ImageInfo verifySignature(InputStream fis) throws IOException {
        byte[] header = getDefaultHeader(fis);
        boolean supported = ((header[0] == (byte) 0xff)
                    && (header[1] == (byte) 0xd8));
        if (supported) {
            ImageInfo info = getDimension(fis);
    ...
    private ImageInfo getDimension(InputStream imageStream) throws IOException {
            ...
            while (true) {
                ...
                switch (marker) {
                    ...
                    case APP0:
                        ...
                        if (densityUnits == 2) {
                            info.setDpiHorizontal(xdensity * 28.3464567 / 72); //dpi
                            info.setDpiVertical(ydensity * 28.3464567 / 72); //dpi
                        } else if (densityUnits == 1) {
                        	info.setDpiHorizontal( xdensity);
                        	info.setDpiVertical( ydensity);
                        } else {
                            //nop, nyi --> 72dpi
                        }
                    ...
                    case SOFA:
                        ...
                        pos+=3; avail-=3;
                        info.setHeight(this.read2bytes(imageStream));
                        pos+=2; avail-=2;
                        info.setWidth(this.read2bytes(imageStream));
                        pos+=2; avail-=2;
                        break outer;
                    default:
                        while (avail < 2) {
