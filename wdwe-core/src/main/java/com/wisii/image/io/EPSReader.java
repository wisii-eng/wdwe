 ...
 *//* $Id: EPSReader.java,v 1.1 2007/04/12 06:41:19 cvsuser Exp $ */
 ...
 *
 * @version   $Id: EPSReader.java,v 1.1 2007/04/12 06:41:19 cvsuser Exp $
 */
public class EPSReader implements ImageReader
{
	...
	// private static final byte[] HIRESBOUNDINGBOX =
	// "%%HiResBoundingBox: ".getBytes();
	// TODO Implement HiResBoundingBox, ImageInfo probably needs some changes
	// for that
	...
	public ImageInfo verifySignature(InputStream bis) throws IOException
	{
		...
		if (isEPS)
		{
			...
			if (data.bbox != null)
			{
				info.setWidth((int) (data.bbox[2] - data.bbox[0]));
				info.setHeight((int) (data.bbox[3] - data.bbox[1]));
	 ...
	 *            The InputStream
	 * @param data
	 *            EPSData object to write the results to
	 * @exception IOException
	 *                If an I/O error occurs
	 */
	private void readEPSImage(InputStream bis, EPSData data) throws IOException
	{
		...
		{
			throw new IOException("载入EPS图片时出错: " + ex.getMessage());
		}
	 ...
	 */
	public byte[] getPreview(EPSData data)
	{
		if (data.preview == null)
		{
			if (data.tiffLength > 0)
			{
				...
				System.arraycopy(data.rawEps, (int) data.tiffStart,
						data.preview, 0, (int) data.tiffLength);
			}
		}
		return data.preview;
	}
	 ...
	 */
	private long[] readBBox(EPSData data)
	{
		long[] mbbox = null;
		int idx = 0;
		boolean found = false;
	...
	private int readLongString(EPSData data, long[] mbbox, int i, int idx)
	{
