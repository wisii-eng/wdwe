...
public class JTaskPane extends JComponent implements Scrollable {
   ...
   * 获得指定位置的Group对象
   * @param index	指定索引
   * @return	{@link JTaskPaneGroup}	返回索引位置的对象。
   */
  public JTaskPaneGroup get(int index){
	 Component comp  = super.getComponent(index);
	 if(!(comp instanceof JTaskPaneGroup))
		 return null;
	 return (JTaskPaneGroup)comp;
  }
  /**
