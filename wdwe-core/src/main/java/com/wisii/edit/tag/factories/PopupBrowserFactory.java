...
/**
 * 这个工厂类负责解析弹出网页控件的有关并生成Swing组件的属性
 * @author xieli
 * @version 1.0 2016/10/11
 *
 */
public enum PopupBrowserFactory implements TagFactory {

	Instance;
	WdemsTagComponent wtc;
...
//		System.out.println("inside select factory: " + tagObject);
		...
		PopupBrowser pb = null;
		
		if (!(tagObject instanceof PopupBrowser))
			return null;
		pb = (PopupBrowser) tagObject;
		wtc = (WdemsTagComponent) new WdemsPopupBrowserCom(pb);
		return wtc;
		
//		return testCombobox();
	}
	
	public WdemsTagComponent getWtc() {
		return wtc;
	}
