 ...
 */
public enum InputFactory implements TagFactory {
...
//		System.out.println("inside input factory: " + tagObject);
		...
		if (tagObject instanceof Input) {
			Input in = (Input) tagObject;
			
			String type = in.getType();
//			String warp = in.getWrap();
			boolean mul = in.isMultiline();
			
			WdemsTagComponent tag = null;
			if (type.equals("password")){
				tag = new PasswordInput(in);
			}else if (type.equals("text")) {
				if (mul){
					tag = new MultiLineInput(in);
				}else{
					tag = new SingleLineInput(in);
				}
			}else{
				tag = new SingleLineInput(in);
			}
