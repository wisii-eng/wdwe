 ...
 * Once a page is rendered it is cleared to release the contents but the PageViewport is retained.
 * So even though the pages are stored the contents are discarded.
 */
public class RenderPagesModel extends AreaTreeModel
{
	 ...
	 * @throws FOVException if the renderer cannot be properly initialized
	 */
	public RenderPagesModel(FOUserAgent userAgent, String outputFormat, FontInfo fontInfo, OutputStream stream) throws FOVException
	{
		super();
		super.setFoUserAgent(userAgent);
		renderer = userAgent.getRendererFactory().createRenderer(userAgent, outputFormat);//AWTRender or PrintRender
		try
		{
			...
			if (!fontInfo.isSetupValid())
				throw new FOVException("No default font defined by OutputConverter");
	...
	/**
	 * Add a page to the render page model. If the page is finished it can be rendered immediately. If the page needs
	 * resolving then if the renderer supports out of order rendering it can prepare the page. Otherwise the page is added to a queue.
	...
	public void addPage(PageViewport page)
	{
		...
		// for links the renderer needs to prepare the page. it is more appropriate to do this after queued pages but
		// it will mean that the renderer has not prepared a page that could be referenced.
		...
		if (ready)
		{
			...
			{
				//TODO use error handler to handle this FOV or IO Exception or propagate exception
	 ...
	 * @param renderUnresolved render pages with unresolved idref's (done at end-of-document processing)
	 * @return true if the current page should be rendered, false if the renderer doesn't support out of order
			...
			if (p.isResolved() || renderUnresolved)
			{
				...
				try
				{
					...
					if (!p.isResolved())
					{
						...
						{
							log.warn("Page " + p.getPageNumberString() + ": Unresolved id reference \"" + idrefs[count] + "\" found.");
						}
					}
				}
				catch (Exception e)
				{
					// use error handler to handle this FOV or IO Exception
					log.error(e);
				}
				p.clear();
				iter.remove();
	...
	/**
	 * Prepare a page.
	 * An unresolved page can be prepared if the renderer supports it and the page will be rendered later.
	...
	/**
	 * @see com.wisii.component.createareatree.area.AreaTreeModel#handleOffDocumentItem(OffDocumentItem)
	 */
	 ...
	 * @see com.wisii.component.createareatree.area.AreaTreeModel#endDocument()
	 */
