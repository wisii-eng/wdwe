 ...
 *//* $Id: Span.java,v 1.1 2007/04/12 06:41:18 cvsuser Exp $ */
...
public class Span extends Area {
    ...
    private void createNormalFlows() {
        ...
        for (int i = 0; i < colCount; i++) {
			...
			newFlow.setXOffset((colWidth + colGap) * i);
			newFlow.setParentArea(this);
    ...
    public NormalFlow getNormalFlow(int colRequested) {
        if (colRequested >= 0 && colRequested < colCount) {
            ...
            throw new IllegalArgumentException("无效的列数 "
                    + colRequested + " 要求；唯一" + (colCount - 1)
                    + " 可用.");
        }
    }
    ...
    public NormalFlow moveToNextFlow() {
        if (hasMoreFlows()) {
            ...
            throw new IllegalStateException("（内部错误），已经没有内容需要处理.");
        }
    }
