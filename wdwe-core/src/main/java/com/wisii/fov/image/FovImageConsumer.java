 ...
 *//* $Id: FovImageConsumer.java,v 1.1 2007/04/12 06:41:19 cvsuser Exp $ */
...
/**
 * ImageConsumer implementation for FovImage classes.
 * @author Eric SCHAEFFER
 */
public class FovImageConsumer implements ImageConsumer {
     ...
     */
    public FovImageConsumer(ImageProducer iprod) {
        this.ip = iprod;
    }
    ...
    public boolean isImageReady() throws Exception {
        ...
        synchronized (this.imageStatus) {
            if (this.imageStatus.intValue() == ImageConsumer.IMAGEABORTED) {
                throw new Exception("图片丢失");
            }
            if (this.imageStatus.intValue() == ImageConsumer.IMAGEERROR) {
                throw new Exception("生成图片时出现错误");
            }
