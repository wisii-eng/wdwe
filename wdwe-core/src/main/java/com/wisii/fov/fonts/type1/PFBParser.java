 ...
 *//* $Id: PFBParser.java,v 1.1 2007/04/12 06:41:19 cvsuser Exp $ */
...
public class PFBParser {
    ...
    static {
        try {
            ...
            throw new RuntimeException("VM不支持US-ASCII 编码");
        }
    }
    ...
    private void parsePCFormat(PFBData pfb, DataInputStream din) throws IOException {
        ...
        if (segmentHead != 128) {
			throw new IOException("无效的文件格式，要求ASCII 80hex");
        }
        ...
        segmentType = din.readUnsignedByte();
        if (segmentType != 3) {
            throw new IOException("segmentType类型异常: " + segmentType);
        }
    }
