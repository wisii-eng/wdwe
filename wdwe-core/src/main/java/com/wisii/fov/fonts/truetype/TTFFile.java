 ...
 *//* $Id: TTFFile.java,v 1.1 2007/04/12 06:41:19 cvsuser Exp $ */
...
import com.wisii.fov.fonts.Glyphs;
...
public class TTFFile {
        ...
        if (!checkTTC(in, name)) {
            if (name == null) {
                throw new IllegalArgumentException(
                    "指定查找的字体为必须在TrueType集合中(-ttcname)");
            } else {
                throw new IOException(
                    "TrueType集合中不存在名字: " + name);
            }
        }
     ...
     * same problem is in the OS/2 table. FOV needs the ascender and descender to determine the
    ...
    private void determineAscDesc() {
        ...
        if (ascender - descender > upem) {
            log.warn("Ascender and descender together are larger than the em box."
                    + " This could lead to a wrong baseline placement in Apache FOV.");
        }
    }
