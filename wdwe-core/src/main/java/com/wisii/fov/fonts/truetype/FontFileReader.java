 ...
 *//* $Id: FontFileReader.java,v 1.1 2007/04/12 06:41:19 cvsuser Exp $ */
...
public class FontFileReader {
    ...
    public void seekSet(long offset) throws IOException {
        if (offset > fsize || offset < 0) {
            throw new java.io.EOFException("无效的 EOF, 文件尺寸=" + fsize
                                           + " offset=" + offset);
        }
        current = (int)offset;
    }
    ...
    public byte read() throws IOException {
        if (current > fsize) {
            throw new java.io.EOFException("文件读取结束, 文件大小=" + fsize);
        }
    ...
    public final void writeTTFUShort(int pos, int val) throws IOException {
        if ((pos + 2) > fsize) {
            throw new java.io.EOFException("无效的 EOF");
        }
    ...
    public final String readTTFString() throws IOException {
        ...
        while (file[i++] != 0) {
            if (i > fsize) {
                throw new java.io.EOFException("无效的 EOF, 文件尺寸="
                                               + fsize);
            }
        }
    ...
    public byte[] getBytes(int offset,
        ...
        if ((offset + length) > fsize) {
            throw new java.io.IOException("无效的 EOF");
        }
