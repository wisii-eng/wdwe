...
import com.wisii.fov.fo.ElementMappingRegistry;
import com.wisii.fov.hyphenation.HyphenationTreeResolver;
...
import com.wisii.fov.util.ContentHandlerFactoryRegistry;

/**
 * Factory class which instantiates new Fov and FOUserAgent instances. This class also holds
 * environmental information and configuration used by FOV. Information that may potentially be
 * different for each rendering run can be found and managed in the FOUserAgent.
 */
public class FovFactory
{
	/** Defines the default source resolution (72dpi) for FOV */
	...
	/** logger instance */
	private static Log log = LogFactory.getLog(FovFactory.class);
	...
	/** The registry for ContentHandlerFactory instance 目前为空*/
	...
	/**
	 * FOV has the ability, for some FO's, to continue processing even if the input XSL violates that FO's content model.
	 * This is the default behavior for FOV.  However, this flag, if set, provides the user the ability for FOV to halt
	 * on all content model violations if desired.
	 */
	...
	/** Main constructor. */
	protected FovFactory()
	{
		this.elementMappingRegistry = new ElementMappingRegistry(this);
	}

	/**
	 * Returns a new FovFactory instance.
	 * @return the requested FovFactory instance.
	 */
	public static FovFactory newInstance()
	{
		return new FovFactory();
	}

	/**
	 * Returns a new FOUserAgent instance. Use the FOUserAgent to configure special values that are particular to a
	 * rendering run. Don't reuse instances over multiple rendering runs but instead create a new one each time and
	 * reuse the FovFactory.
	...
	/**
	 * Returns a new {@link Fov} instance. Use this factory method if you want to configure this very rendering run,
	 * i.e. if you want to set some metadata like the title and author of the document you want to render. In that case,
	 * create a new {@link FOUserAgent} instance using {@link #newFOUserAgent()}.
	 * <p>
	 ...
	 * @return the new Fov instance
	 * @throws FOVException  when the constructor fails
	 */
	public Fov newFov(String outputFormat, FOUserAgent userAgent) throws FOVException
	{
		if (userAgent == null)
			throw new NullPointerException("userAgent参数不能为空!");
		return new Fov(outputFormat, userAgent);
	}

	/**
	 * Returns a new {@link Fov} instance. Use this factory method if your output type requires an output stream and you
	 * want to configure this very rendering run, i.e. if you want to set some metadata like the title and author of the
	 * document you want to render. In that case, create a new {@link FOUserAgent} instance using {@link #newFOUserAgent()}.
	 * <p>
	 * MIME types are used to select the output format (ex. "application/pdf" for PDF). You can
	 * use the constants defined in {@link MimeConstants}.
	 * @param outputFormat the MIME type of the output format to use (ex. "application/pdf").
	 ...
	 * @throws FOVException when the constructor fails
	 */
	public Fov newFov(String outputFormat, FOUserAgent userAgent, OutputStream stream) throws FOVException
	{
		if (userAgent == null)
			throw new NullPointerException("userAgent参数不能为空!");
		return new Fov(outputFormat, userAgent, stream);
	}
	...
	/**
	 * Activates strict XSL content model validation for FOV
	 * Default is false (FOV will continue processing where it can)
	 ...
	 * <p>
	 * This flag controls whether FOV will enable special code that breaks property inheritance for start-indent and
	 * end-indent when the evaluation of the inherited value would cross a reference area. This is described under
	 * http://wiki.apache.org/xmlgraphics-fov/IndentInheritance as is intended to improve interoperability with commercial
	 * FO implementations and to produce results that are more in line with the expectation of unexperienced FO users.
	...
	/**
	 * Adds a namespace to the set of ignored namespaces. If FOV encounters a namespace which it cannot handle, it issues
	 * a warning except if this namespace is in the ignored set.
	 ...
	 * If FOV encounters a namespace which it cannot handle, it issues a warning except if this namespace is in the ignored set.
	 ...
	 * @param namespaceURI the namespace URI
	 * @return true if the namespace is ignored by FOV
	 */
	...
	/** @return the set of namespaces that are ignored by FOV */
	 ...
	 * Will use the configured resolver and if not successful fall back to the default resolver.
	 * @param uri URI to access
	 * @param base the base URI to resolve against
	 * @return A {@link javax.xml.transform.Source} object, or null if the URI cannot be resolved.
	 * @see com.wisii.fov.apps.FOURIResolver
	 */
	...
	/**
	 * Returns a new {@link Fov} instance. FOV will be configured with a default user agent instance.
	 * <p>
	 * MIME types are used to select the output format (ex. "application/pdf" for PDF). You can
	 * use the constants defined in {@link MimeConstants}.
	 * @param outputFormat the MIME type of the output format to use (ex. "application/pdf").
	 ...
	 * @throws FOVException when the constructor fails
	 */
    public Fov newFov(String outputFormat) throws FOVException {
		return new Fov(outputFormat, newFOUserAgent());
	}

/**
 * Returns a new {@link Fov} instance. FOV will be configured with a default user agent
 ...
 * @throws FOVException when the constructor fails
 */
    public Fov newFov(String outputFormat, OutputStream stream) throws FOVException {
	return new Fov(outputFormat, newFOUserAgent(), stream);
}

/**
 * Returns a new {@link Fov} instance. Use this factory method if you want to supply your
 * own {@link com.wisii.fov.render.Renderer Renderer} or
 * {@link com.wisii.fov.fo.FOEventHandler FOEventHandler}
 * instance instead of the default ones created internally by FOV.
 * @param userAgent the user agent that will be used to control the rendering run
 * @return the new Fov instance
 * @throws FOVException when the constructor fails
 */
    public Fov newFov(FOUserAgent userAgent) throws FOVException {
