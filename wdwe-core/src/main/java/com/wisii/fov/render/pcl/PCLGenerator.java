...
public class PCLGenerator {
	...
	public void selectDoublePrint(int selector) throws IOException
	{

		writeCommand("&l" + selector + "S");
	}
	public void selectcmd(String cmd) throws IOException
	{
    ...
    private MonochromeBitmapConverter createMonochromeBitmapConverter() {
        ...
        try {
            converter = com.wisii.fov.render.pcl.JAIMonochromeBitmapConverter.class.newInstance();
        } catch (LinkageError le) {
            // This can happen if fov was build with support for a
            // particular provider (e.g. a binary fov distribution)
