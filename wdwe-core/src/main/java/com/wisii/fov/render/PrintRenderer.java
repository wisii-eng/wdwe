...
// del by huangzl.精简客户端的jar包。实际没有使用FontResolver和FontSetup
...
public abstract class PrintRenderer extends AbstractRenderer
{
    ...
    public void setupFontInfo(FontInfo inFontInfo)
    {
        ...
        // del by huangzl.精简客户端的jar包
        //由于需要生成pdf.ps等文件所以不删除以下2行代码
        FontResolver resolver = new DefaultFontResolver(userAgent);
        FontSetup.setup(fontInfo, fontList, resolver, userAgent.getFactory().isBase14KerningEnabled());
    }
    ...
    protected RendererContext createRendererContext(int x, int y, int width, int height, Map foreignAttributes)
    {
        ...
        context.setProperty(RendererContextConstants.HEIGHT, new Integer(height));
        ...
        context.setProperty(RendererContextConstants.PAGE_VIEWPORT, getCurrentPageViewport());
