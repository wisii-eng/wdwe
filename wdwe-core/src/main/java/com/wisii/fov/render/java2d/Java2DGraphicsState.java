 ...
 *//* $Id: Java2DGraphicsState.java,v 1.2 2007/07/04 05:51:36 hzl Exp $ */
...
public class Java2DGraphicsState {
    ...
    public boolean updateFont(String name, int size) {
        ...
        boolean updateStyle = (mapper.getStyle() != getGraph().getFont().getStyle()); // add by huangzl.因为style变化，但getGraph().getFont().getStyle()的返回值可能不变。

        if (updateName || updateSize || updateStyle) {
