...
/* $Id: FoUnitsConverter.java 426576 2006-07-28 15:44:37Z jeremias $ */
 ...
 *  for the JFOR project and is now integrated into FOV.
 */
...
final class FoUnitsConverter {
    ...
    private float numberToTwips(String number, String units)
        ...
        try {
            ...
            throw new FOVException("number format error: cannot convert '"
        ...
        if (units != null && units.trim().length() > 0) {
            ...
            if (factor == null) {
                throw new FOVException("conversion factor not found for '" + units + "' units");
            }
    ...
    int convertFontSize(String size) throws FOVException {
        size = size.trim();
        final String sFONTSUFFIX = "pt";
        if (!size.endsWith(sFONTSUFFIX)) {
            throw new FOVException("Invalid font size '" + size + "', must end with '"
                                   + sFONTSUFFIX + "'");
        }
        ...
        try {
            ...
            throw new FOVException("Invalid font size value '" + size + "'");
        }
