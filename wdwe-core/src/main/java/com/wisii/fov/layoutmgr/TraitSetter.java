...
import com.wisii.fov.image.AbstractFovImage;
...
public class TraitSetter {
    ...
    public static void addBackground(Area area,
		...
		area.addTrait(Trait.IMAGE_LAYER, new Integer(backProps.getBackgroundLayer()));
        ...
        if (backProps.getFovImage() != null) {
                    ...
                    if (area.getIPD() > 0) {
                        ...
                        back.setHoriz(backProps.backgroundPositionHorizontal.getValue(
                                new SimplePercentBaseContext(context,
                                    ...
                                    (width - back.getFovImage().getIntrinsicWidth())
                                )
                            ));
                    } else {
                    ...
                    if (area.getBPD() > 0) {
                        ...
                        back.setVertical(backProps.backgroundPositionVertical.getValue(
                                new SimplePercentBaseContext(context,
                                     ...
                                     (height - back.getFovImage().getIntrinsicHeight())
                                )
                            ));
                    } else {
            ...
            ((AbstractFovImage)(back.getFovImage())).setInputStream(null); // 因为InputStream不支持序列化,所以使用结束后，设置为null
        }
