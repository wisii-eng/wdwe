 ...
 *//* $Id: AbstractBreaker.java,v 1.2 2007/09/19 09:14:03 hzl Exp $ */
...
//                System.err.println("TIME_创建Knuth Elements开始。           时间（毫秒）：" + times);
            }
            nextSequenceStartsOn = getNextBlockList(childLC, nextSequenceStartsOn, blockLists);
            if(SystemUtil.PRINT_RUN_TIME)
            {
                timesEnd = System.currentTimeMillis();
//                System.err.println("TIME_创建Knuth Elements结束。           时间（毫秒）：" + timesEnd);
                System.err.println("TIME_创建Knuth Elements。消耗时间（毫秒）：" + (timesEnd - times));
            }
            ...
            for (blockListIndex = 0; blockListIndex < blockLists.size(); blockListIndex++) {
                ...
                {
                    times = System.currentTimeMillis();
//                    System.err.println("TIME_分页计算开始。           时间（毫秒）：" + times);
                }
                ...
                {
                    timesEnd = System.currentTimeMillis();
//                    System.err.println("TIME_分页计算结束。           时间（毫秒）：" + timesEnd);
                    System.err.println("TIME_分页计算。消耗时间（毫秒）：" + (timesEnd - times));
                }
                ...
                {
                    times = System.currentTimeMillis();
//                    System.err.println("TIME_创建Area开始。           时间（毫秒）：" + times);
                }
                doPhase3(alg, iOptPageCount, blockList, effectiveList);
                if(SystemUtil.PRINT_RUN_TIME)
                {
                    timesEnd = System.currentTimeMillis();
//                    System.err.println("TIME_创建Area结束。           时间（毫秒）：" + timesEnd);
                    System.err.println("TIME_创建Area。消耗时间（毫秒）：" + (timesEnd - times));
                }
            }
        }
    }
        ...
        if ((returnedList = getNextKnuthElements(childLC, alignment)) != null) {
            ...
            if (((KnuthElement) returnedList.getLast()).isPenalty()
                ...
                case Constants.EN_EVEN_PAGE:
                    ...
                    break;
                default:
                    throw new IllegalStateException(" 非法类: "
                            + breakPenalty.getBreakClass()+" 中断");
                }
            }
