 ...
 *//* $Id: TableContentLayoutManager.java,v 1.4 2007/08/29 05:29:59 hzl Exp $ */
...
import com.wisii.fov.layoutmgr.ElementListObserver;
import com.wisii.fov.layoutmgr.ElementListUtils;
...
import com.wisii.fov.layoutmgr.MinOptMaxUtil;
import com.wisii.fov.layoutmgr.Position;
import com.wisii.fov.layoutmgr.PositionIterator;
import com.wisii.fov.layoutmgr.SpaceResolver;
...
public class TableContentLayoutManager implements PercentBaseContext {
			...
			EffRow[] rowGroup)
	{
		...
		for (int rgi = 0; rgi < rowGroup.length; rgi++)
		{
			...
			{
				maxColumnCount = Math.max(maxColumnCount, row.getGridUnits()
						.size());
				GridUnit gu = row.getGridUnit(j);
				if ((gu.isPrimary() || (gu.getColSpanIndex() == 0 && gu
						...
						&& !gu.isEmpty())
				{
					...
					if (gu.isPrimary())
					{
						...
						if (tableRow == null && primary.getRow() != null)
						{
							...
							// Check for bpd on row, see CSS21, 17.5.3 Table
							// height algorithms
							LengthRangeProperty bpd = tableRow
									.getBlockProgressionDimension();
							if (!bpd.getMinimum(getTableLM()).isAuto())
							{
								minContentHeight = Math.max(minContentHeight,
										bpd.getMinimum(getTableLM())
												.getLength().getValue(
														getTableLM()));
							}
							MinOptMaxUtil.restrict(explicitRowHeights[rgi],
									bpd, getTableLM());

						}
						// Calculate width of cell
						int spanWidth = 0;
						for (int i = primary.getStartCol(); i < primary
								.getStartCol()
								+ primary.getCell().getNumberColumnsSpanned(); i++)
						{
							if (getTableLM().getColumns().getColumn(i + 1) != null)
							{
								spanWidth += getTableLM().getColumns()
										.getColumn(i + 1).getColumnWidth()
										.getValue(getTableLM());
							}
						}
						...
						LinkedList elems = primary.getCellLM()
								.getNextKnuthElements(childLC, alignment);
						// Temporary? Multiple calls in case of break
						// conditions.
						// TODO Revisit when table layout is restartable
						while (!primary.getCellLM().isFinished())
						{
							LinkedList additionalElems = primary.getCellLM()
									.getNextKnuthElements(childLC, alignment);
						...
						if ((elems.size() > 0)
								&& ((KnuthElement) elems.getLast())
										.isForcedBreak())
						{
							// a descendant of this block has break-after
							log
									.debug("Descendant of table-cell signals break: "
						...
						{
							log
									.debug("child LM signals pending keep-with-next");
							primary.setFlag(GridUnit.KEEP_WITH_NEXT_PENDING,
									true);
						}
						if (childLC.isKeepWithPreviousPending())
						{
							log
									.debug("child LM signals pending keep-with-previous");
							primary.setFlag(
									GridUnit.KEEP_WITH_PREVIOUS_PENDING, true);
						}
					}

					// Calculate height of cell contents
					primary.setContentLength(ElementListUtils
							.calcContentLength(primary.getElements()));
					maxCellHeight = Math.max(maxCellHeight, primary
							.getContentLength());

					// Calculate height of row, see CSS21, 17.5.3 Table height
					// algorithms
					if (gu.isLastGridUnitRowSpan())
					{
						int effCellContentHeight = minContentHeight;
						LengthRangeProperty bpd = primary.getCell()
								.getBlockProgressionDimension();
						if (!bpd.getMinimum(getTableLM()).isAuto())
						{
							effCellContentHeight = Math.max(
									effCellContentHeight, bpd.getMinimum(
											getTableLM()).getLength().getValue(
											getTableLM()));
						}
						if (!bpd.getOptimum(getTableLM()).isAuto())
						{
							effCellContentHeight = Math.max(
									effCellContentHeight, bpd.getOptimum(
											getTableLM()).getLength().getValue(
											getTableLM()));
						}
						if (gu.getRowSpanIndex() == 0)
						{
							// TODO ATM only non-row-spanned cells are taken for
							// this
							MinOptMaxUtil.restrict(explicitRowHeights[rgi],
									bpd, tableLM);
						}
						...
						if (isSeparateBorderModel())
						{
							borderWidths = primary.getBorders()
									.getBorderBeforeWidth(false)
									+ primary.getBorders().getBorderAfterWidth(
											false);
						} else
						{
						...
						padding += cbpb.getPaddingBefore(false, primary
								.getCellLM());
						padding += cbpb.getPaddingAfter(false, primary
								.getCellLM());
						int effRowHeight = effCellContentHeight + padding
								+ borderWidths + 2
								* getTableLM().getHalfBorderSeparationBPD();
						...
						if (effRowHeight > rowHeights[rgi].min)
						{
							...
							MinOptMaxUtil.extendMinimum(rowHeights[rgi],
									effRowHeight, false);
						}
					}
						...
						.warn(FONode
								.decorateWithContextInfo(
										"The contents of row "
												...
												+ " to "
												+ effRowContentHeight
												+ " millipoints, but the row shouldn't get"
												+ " any taller than "
		...
		// TODO It may make sense to reuse the stepper since it allocates quite
		// some space
            ...
            while (partIter.hasNext()) {
                ...
                if (gridUnits[colIndex] != gup.pgu) {
                    ...
                    if (gup.end < end[colIndex]) {
                        throw new IllegalStateException("内部错误: stepper问题");
                    }
                    end[colIndex] = gup.end;
                }
            }
        ...
        public int addAreasAndFlushRow(boolean forcedFlush) {
            ...
            for (int i = 0; i < gridUnits.length; i++) {
                ...
                if ((gridUnits[i] != null)
                    ...
                    if (gu != null) {
                        addAreasForCell(gu, start[i], end[i], lastRow, partLength[i], actualRowHeight);
                        gridUnits[i] = null;
                        start[i] = 0;
                        end[i] = -1;
                        partLength[i] = 0;
    ...
    public static class TableContentPosition extends Position {
        ...
        /** @see com.wisii.fov.layoutmgr.Position#generatesAreas() */
